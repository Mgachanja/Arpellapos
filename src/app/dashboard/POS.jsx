...
function ProductsGrid({ hasSearched, filteredProducts, searchTerm, isLikelyBarcode, cartMap, onQuantityChange, loadingProducts }) {
  if (!hasSearched) {
    return (
        ...
        return (
          <div key={productId} className="col-6 col-sm-4 col-md-6 col-lg-4 col-xl-3 mb-3">
            <div style={{ position: 'relative' }}>
              <ProductCard product={product} cartQuantity={cartMap[productId] || 0} onQuantityChange={onQuantityChange} />
              {isLoading && (
            ...
            return (
              <tr key={itemId}>
                ...
                <td className="text-center">
                  <button className="btn btn-outline-danger btn-sm rounded-circle" onClick={() => onRemoveItem(itemId, item.name)} title={`Remove ${item.name}`} style={{ width: '24px', height: '24px', padding: '0', fontSize: '0.6rem' }}>
                    <i className="fas fa-times"></i>
                  </button>
                </td>
              </tr>
  ...
  return (
    <div className="container-fluid py-4" style={{ background: '#f8f9fa', minHeight: '100vh', maxWidth: '100%', overflow: 'hidden' }}>
      <div className="row h-100" style={{ minHeight: 'calc(100vh - 2rem)' }}>
        {/* Left side - Products (1/4 of screen) */}
                ...
                >
                  {processingOrder ? (
                    ...
                    <>
                      ...
                      {paymentType === 'both' ? 'Create Order' : 'Complete Order'} - {KSH(cartTotal)}
                    </>
                  )}
                </Button>
              </div>
      ...
      <style jsx>{`
        ...
        @media (min-width: 1200px) {
          .col-xl-3 { flex: 0 0 33%; max-width: 33%; }
          .col-xl-9 { flex: 0 0 67%; max-width: 67%; }
        }
      `}</style>
    </div>
  );
