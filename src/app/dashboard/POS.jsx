...
const ProductCard = React.memo(function ProductCard({ product, cartQuantity, onQuantityChange }) {
  const productId = product.id || product._id;

  const handleIncrement = useCallback(() => {
    onQuantityChange(productId, (cartQuantity || 0) + 1);
  }, [productId, cartQuantity, onQuantityChange]);
...
export default function POS() {
  ...
  }, [cartMap, getInventoryId, setProductLoading, dispatch]);
  ...
  }, [filteredProducts, cartMap, getInventoryId, setProductLoading, dispatch]);
  ...
  const handleOrderCompletion = useCallback(async (orderData) => {
    ...
    // Create receipt data with EXACT structure expected by printer
    const receiptData = {
      cart: cart,
      cartTotal: cartTotal,
      ...
      paymentData: {
        ...
        mpesaPhone: paymentType === 'mpesa' || paymentType === 'both' ? (paymentData.mpesaPhone || '').trim() : ''
      },
      user: user,
  ...
  }, [cart, cartTotal, paymentType, paymentData, user, dispatch]);
  ...
  }, [paymentType, paymentData, cartTotal, user, coords, cart, handleOrderCompletion]);
    ...
    <div className="container-fluid py-4" style={{ background: '#f8f9fa', minHeight: '100vh', maxWidth: '100%', overflow: 'hidden' }}>
      <div className="row h-100" style={{ minHeight: 'calc(100vh - 2rem)' }}>
        ...
        <div className="col-lg-8 col-12 mb-4">
          ...
          {/* Products Grid */}
          <div className="products-container" style={{ maxHeight: 'calc(100vh - 200px)', overflowY: 'auto', paddingRight: '20px' }}>
            <div className="row">
              {!hasSearched ? (
              ...
              <div className="cart-checkout border-top pt-3">
                ...
                {paymentType === 'cash' && (
                  <Form.Group className="mb-3">
                    ...
                    <div className="input-group input-group-lg">
                      <span className="input-group-text" style={{ fontSize: '1.1rem' }}>Ksh</span>
                      <Form.Control 
                        ref={cashInputRef}
                        type="number" 
                ...
                {paymentType === 'mpesa' && (
                  <Form.Group className="mb-3">
                    ...
                    <div className="input-group input-group-lg">
                      <span className="input-group-text" style={{ fontSize: '1.1rem' }}>📱</span>
                      <Form.Control 
                        ref={phoneInputRef}
                        type="tel" 
