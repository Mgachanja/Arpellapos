...
export default function POS() {
  ...
  const handleBarcodeScanned = useCallback(async (barcode) => {
    try {
      ...
      // Keep the product visible by storing it in barcodeResultsRef
      barcodeResultsRef.current = [product];
      setFilteredProducts([product]);
      setHasSearched(true);
      setSearchType('barcode');

      // Clear search term but keep product visible
      setSearchTerm('');
      ...
      toast.success(`${product.name} - Ready to add to cart`, { autoClose: 2000 });
    } catch (error) {
  ...
  const performSearch = useCallback(async (term) => {
    // If search term is empty, don't clear the barcodeResultsRef (keeps scanned product visible)
    if (!term || term.trim().length === 0) {
      // Only clear if there's no barcode result to maintain
      if (!barcodeResultsRef.current) {
        setFilteredProducts([]); 
        setHasSearched(false); 
        setSearchType(''); 
      }
  ...
  useEffect(() => {
    // Only clear barcodeResultsRef if user is actively typing (not when it's empty from barcode scan)
  ...
  }, [filteredProducts, cart, getInventoryId, setProductLoading, dispatch, focusSearchInput]);
  ...
  const handleCheckoutSale = useCallback(async (saleId, checkoutData) => {
    try {
      ...
      // Clear current cart and add held sale items
      dispatch(clearCart());
      if (Array.isArray(sale.items)) {
        sale.items.forEach(item => {
          dispatch(addItemToCart({ 
      ...
      // Set payment info
      setPaymentType(checkoutData.paymentType);
      setPaymentData(checkoutData.paymentData);

      // Wait for cart to update
      await new Promise(resolve => setTimeout(resolve, 100));

      // Process the order
      if (checkoutData.paymentType === 'both') {
        await createOrder();
      } else {
        await completeCheckout();
      }

      // Delete the held sale after successful checkout
      heldSalesService.deleteHeldSale(saleId);
      const updatedSales = heldSalesService.getAllHeldSales();
      setHeldSales(Array.isArray(updatedSales) ? updatedSales : []);
      ...
      toast.error('Failed to checkout sale');
    }
  }, [dispatch, createOrder, completeCheckout]);

  const currentCartTotal = calculateCartTotal();
  
  // Keep barcode results visible while search is empty
  const displayedProducts = (barcodeResultsRef.current && !searchTerm.trim()) 
